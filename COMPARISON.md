...
We have a lean coding-agent CLI (this repo — Effect.ts + Bun, ports/adapters, on `@effect/ai` with a multi-provider router). It *feels* like it's missing a lot next to mature agents — better tool showcasing, diffs, web search, extensibility, compaction, memory references. This is an in-depth, area-by-area comparison **so we understand the difference**, plus a prioritized build roadmap.
...
| **Web search / grounding** | ❌ | ❌ (provider-side only) | ✅ `WebSearchTool` | native `GoogleSearch`/`WebSearch` tools, but need a provider-aware toolkit. **Low-med** | **BUILD** (next) |
...
| **Skills** | ✅ `.agent/skills` + `read_skill` | ✅ `.pi/skills` | ✅ bundled+local+MCP | — | keep |
...
1. **pi shares our "skips."** No web tools, no sub-agents, no todo, no MCP, no permission gates, no @-mentions — a real peer agent deliberately omits exactly what we marked SKIP/MAYBE. Good signal our verdicts are sane (Claude Code's breadth is the outlier, not our leanness).
...
- **Web search "now on Effect."** Confirmed: both providers ship **native** search tools in the SDK (`GoogleTool.GoogleSearch` + `UrlContext`; `OpenAiTool.WebSearch`), provider-executed (`requiresHandler:false`). They drop straight into a `Toolkit`. The wrinkle: they're provider-specific, so the toolkit must include the right one for the active provider (the router already knows the provider). Plus a generic `web_fetch` over our existing `HttpClient`.
...
1. **Render diffs** for `edit_file`/`write_file` in the TUI (ANSI unified diff). *Files:* `cli/src/tui/{scrollback,toolDescribe,render}.ts`; diff already in `core/usecases/codingToolkit.ts`.
2. **Expandable/scrollable tool output** (toggle full bash/grep/read output). *Files:* `cli/src/tui/{scrollback,render,keys}.ts`, `modes/tui.ts`.
3. **Streaming** — `streamText` per turn; map `StreamPart`s to `AgentEvent`s for token-level text + live tool args. *Files:* `core/usecases/agentLoop.ts`, `core/usecases/promptMapping.ts`, `cli/src/events.ts`, `adapters/src/llm/router.ts` (router already implements `streamText`).
4. **Web search + web_fetch tools** — provider-native search in the toolkit (provider-aware) + a `web_fetch` `Tool.make`. *Files:* new `core/usecases/webToolkit.ts` (or extend `codingToolkit.ts`), `adapters/src/llm/router.ts` (provider-aware tool selection).
...
We're not far behind on *capability foundations* — the loop, multi-provider routing, tools, skills, instruction files, and persisted history are solid. The visible gaps are mostly **presentation** (diffs, expandable output, streaming) and a few **high-value tools** (web search, sub-agents, todo) — all low/medium effort on the Effect stack, and most are strong build-in-public content. The genuinely large, off-thesis items (plugins, Team mode, cron/remote, LSP) we skip on purpose: a lean Effect agent doesn't need to be Claude Code to be sharp at its wedge.

Measured against **pi** — our true peer — the gap is narrower and clearer: we match its tool set and share its deliberate omissions, but trail on **rendered diffs, collapsible/streaming tool UI, real compaction, session branching, and a runtime extension system.** Close those (Tier 1/2) and we're at peer level on UX, with the Effect substrate (extensions-as-Layers, colocated evals) as the wedge pi doesn't have.
