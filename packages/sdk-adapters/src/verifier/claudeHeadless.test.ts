...
describe("parseDeliverableVerdict", () => {
  ...
  it("tolerates `reasons` as a bare STRING — the shape that broke the structured verifier", () => {
    // Opus returned `reasons: ""` (string, not array); the strict generateObject
    // schema rejected it → gate "unavailable". Here it decodes and normalizes.
    expect(parseDeliverableVerdict('{"verdict": "sound", "reasons": ""}')!.reasons).toEqual([])
    expect(parseDeliverableVerdict('{"verdict": "needs_work", "reasons": "fix the decoder"}')!.reasons).toEqual([
      "fix the decoder",
    ])
  })

  it("finds the verdict after a BRACE-HEAVY assessment — the shape that broke the greedy regex", () => {
    // A real Opus review: prose full of `{ }` code, THEN the verdict object. The
    // old `/\{[\s\S]*\}/` grabbed first-brace-to-last and failed to parse; the
    // balanced last-object scan lands on the trailing verdict.
    const text =
      "Looking at the diff: `const f = (x) => { return { y: x } }` and the handler " +
      "`{ ok: false }` both look off. The reducer `{ a, b }` is fine though.\n\n" +
      '```json\n{"verdict": "needs_work", "reasons": ["fix the handler shape"]}\n```'
    const v = parseDeliverableVerdict(text)
    expect(v!.verdict).toBe("needs_work")
    expect(v!.reasons).toEqual(["fix the handler shape"])
  })

  it("empty output → undefined (the caller then fails → 'unavailable', its true meaning)", () => {
    expect(parseDeliverableVerdict("")).toBeUndefined()
    expect(parseDeliverableVerdict("   \n  ")).toBeUndefined()
  })

  it("present-but-unparseable text degrades to needs_work (fail-CLOSED, never silent fail-open)", () => {
    // No decodable JSON, but there IS output — re-check the work rather than ship
    // it unverified. An EXPLICIT `verdict: <word>` is honored; a loose "sound"
    // buried in prose is NOT (that would risk shipping unverified work).
    const ambiguous = parseDeliverableVerdict("the work looks reasonable to me")!
    expect(ambiguous.verdict).toBe("needs_work")
    expect(ambiguous.reasons.length).toBeGreaterThan(0)
    // loose "sound" in prose → still needs_work (conservative)
    expect(parseDeliverableVerdict('I think it is sound overall.')!.verdict).toBe("needs_work")
    // explicit verdict label → honored
    expect(parseDeliverableVerdict("Final verdict: sound. Ship it.")!.verdict).toBe("sound")
    expect(parseDeliverableVerdict('{"verdict": "maybe"} — actually it is blocked')!.verdict).toBe("blocked")
  })
})
...
describe("ClaudeHeadlessVerifierLive.gate (full adapter path)", () => {
  ...
  it("degrades present-but-unparseable output to needs_work (NOT 'unavailable')", async () => {
    // The whole point of the revert's robust parse: a present but oddly-shaped
    // judgment re-checks the work (fail-closed) instead of silently shipping it.
    const res = await runGate(envelope("the work looks fine to me"))
    expect(res._tag).toBe("Right")
    if (res._tag === "Right") expect(res.right.verdict).toBe("needs_work")
  })

  it("fails (→ 'unavailable') only when claude produces NO output", async () => {
    const res = await runGate(envelope(""))
    expect(res._tag).toBe("Left")
    if (res._tag === "Left") expect(res.left.message).toContain("no output")
  })
