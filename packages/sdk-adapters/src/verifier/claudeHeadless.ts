...
/** The deliverable-gate verdict the model emits (inside `result`). `reasons` is
 *  deliberately TOLERANT — an array, a bare string, or missing all decode (the
 *  model sometimes returns `""` when sound, or a single string) and are
 *  normalized by {@link normReasons}. This is the exact shape that broke the
 *  strict `generateObject` verifier (`reasons: ""` rejected as not-an-array). */
...
/** Normalize the tolerant `reasons` field to a clean string[]: a bare string
 *  becomes one reason (or none if empty); an array is trimmed + emptied-filtered. */
const normReasons = (reasons: ReadonlyArray<string> | string | undefined): ReadonlyArray<string> => {
  if (reasons === undefined) return []
  const arr = typeof reasons === "string" ? [reasons] : reasons
  return arr.map((r) => r.trim()).filter((r) => r.length > 0)
}

/** Clip a fallback reason so a whole transcript never floods the retry message. */
const clip = (s: string, max = 1500): string =>
...
/** Decode the refute verdict. Tries each balanced `{…}` object in the reply
 *  (last-first, so the trailing verdict wins past any brace-heavy reasoning) and
 *  takes the first that validates. `undefined` on a genuine miss → the caller
 *  rejects the candidate (fail-closed: an unverifiable learning is never saved). */
export const parseVerdict = (resultText: string): Verdict | undefined => {
  for (const candidate of extractJsonObjects(resultText)) {
    const v = Either.getOrUndefined(Schema.decodeUnknownEither(VerdictReply)(candidate))
    if (v !== undefined) return { accept: v.accept, score: v.score, reason: v.reason?.trim() ?? "" }
  }
  return undefined
}

/** Find the verdict word in prose when no JSON object decodes — looks for the
 *  `verdict` field value first, else a standalone keyword. Defaults to
 *  `needs_work` (fail-CLOSED: re-check the work) rather than guessing `sound`. */
const keywordVerdict = (text: string): DeliverableVerdict["verdict"] => {
  const m = text.match(/verdict["'\s:]*["']?(sound|needs_work|blocked)/i)
  const hit = m?.[1]
  if (hit !== undefined) return hit.toLowerCase() as DeliverableVerdict["verdict"]
  if (/\bblocked\b/i.test(text)) return "blocked"
  return "needs_work"
}

/**
 * Decode the deliverable-gate verdict — ROBUSTLY. Tries each balanced `{…}`
 * object (last-first) against the tolerant schema; the trailing verdict wins even
 * after pages of brace-heavy assessment (the case the old greedy regex broke on),
 * and `reasons:""`/a bare string decode fine (the case the structured verifier
 * broke on). If NO object decodes but there IS output, a keyword fallback yields a
 * verdict with the raw text as the reason — so a present-but-oddly-shaped judgment
 * degrades to `needs_work`, never the silent fail-open of "verifier unavailable".
 * `undefined` ONLY when the reply is empty (the caller then fails → unavailable,
 * its true meaning: claude produced nothing).
 */
export const parseDeliverableVerdict = (
  ...
  const verdict = keywordVerdict(text)
  return { verdict, reasons: verdict === "sound" ? [] : [clip(text)] }
}
...
const VALIDATOR_CLAUDE_MD =
  ...
  `trust a summary. Be a skeptical referee, not a collaborator. End your reply with the JSON verdict ` +
  `the prompt asks for, as a single fenced \`\`\`json code block.\n`
    ...
    const gate = (
      ...
      loadConstraintIds(input.repoDir).pipe(
        ...
        Effect.flatMap((text) => {
          ...
          // so a shape quirk never reaches here as `unavailable`.
          const verdict = parseDeliverableVerdict(text)
          return verdict === undefined
            ? Effect.fail(new VerifierError({ message: "claude produced no output" }))
            : Effect.succeed(verdict)
        }),
