...
**Routing: one agent = one piece of work.** Default to a FRESH spawn for every new task — even in the same folder. Fresh context is cheaper and more focused; resuming re-feeds the node's entire history on every turn and mixes unrelated work into one context. Reuse a node only when the new task is a direct follow-up on that node's own output: seedFromNode + seedMode "resume" to continue/fix/extend what it just did (its in-context file knowledge is the point), or "branch" to retry / take a different direction from its context without growing the original. Never route a task to an old node just because the folder matches.
