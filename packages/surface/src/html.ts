/**
 * The `html` tagged template — the one way strings become markup on the new
 * line. Interpolated values are escaped by default; only an explicit
 * {@link raw} passes through, and `raw()` may appear ONLY in the sanitizer's
 * output and vendored-asset injection (see CLAUDE.md).
 */
...
/** Values the template accepts: strings/numbers escape; `Html` passes
 *  through; `null | undefined | false` elide; arrays flatten recursively. */
export type Interp =
  | string
  | number
  | Html
...
/** Escape a plain string for HTML text or attribute-value position. */
...
/** Unwrap `Html` to the final string (the seam to socket sends / HTTP bodies). */
...
export const html = (strings: TemplateStringsArray, ...vals: ReadonlyArray<Interp>): Html => ({
  __html: vals.reduce<string>(
    (acc, v, i) => acc + interpToString(v) + (strings[i + 1] ?? ""),
    strings[0] ?? "",
  ),
})
