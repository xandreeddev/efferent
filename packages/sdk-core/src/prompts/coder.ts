...
import { isOrchestrateMode } from "../usecases/buildScopeRuntime.js"
import {
...
export const systemSection = `# System
...
export const doingTasksSection = `# Doing tasks
...
export const toneSection = `# Tone and formatting
...
export const knowledgeSection = `# Knowledge and search
...
export const safetySection = `# Refusals and safety
...
export const actionsSection = `# Executing actions with care
...
export const renderSkillsSection = (skills: ReadonlyArray<Skill>): string => {
...
export const renderToolsSection = (tools: ReadonlyArray<ToolDefinition>): string => {
...
/** The hands-on root's `# Tools` block, shared by the direct coder prompt and
 *  the web-agent prompt (`prompts/web.ts`). `webUi` prepends the `render_ui`
 *  line — the web canvas is that agent's primary output surface, so it leads. */
export const renderCoreToolsSection = (
  skills: ReadonlyArray<Skill>,
  memory: ReadonlyArray<Memory>,
  tools: ReadonlyArray<ToolDefinition>,
  webUi = false,
): string => `# Tools
${webUi ? `- render_ui({ id, title?, html, mode?, active? }) — build or update a PAGE in the user's browser (see '# The web canvas' below). Your primary way to present anything visual, structured, or interactive.\n` : ""}- read_file({ path, offset?, limit? }) — read a file's contents (line-numbered). Use offset/limit on big files.
...
${renderCoreToolsSection(skills, memory, tools)}
