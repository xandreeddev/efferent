...
## Resilience — transient failures don't kill the turn

A model call can fail two ways, and the loop treats them differently. A **transient** failure — a 429
rate-limit, a retryable 5xx (overload / bad gateway / unavailable), a transport error, or a timeout — is
**retried with jittered exponential backoff** (honouring `Retry-After` when present) instead of aborting
the turn. The wrapper is `retryableLlm` (`adapters/src/llm/retry.ts`), applied at the **connect** (never
around a live stream, which would duplicate tokens): it wraps the router's `generateText` /
`generateObject` for **every** provider — including the default Kimi/opencode gateway, which returns 429
"engine overloaded" under load — plus the fast-tier `utilityLlm` and `webSearch`.

**Permanent** errors — a 4xx that isn't 429, or a decode failure — surface immediately; retrying them
would just burn time. So a flaky provider no longer ends a turn (or thrashes a whole fleet into
re-spawning), while a genuine bad request still fails fast.
