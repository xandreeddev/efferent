...
`sdk-core/usecases/agentBus.ts` (`AgentBus`, aliased `Supervisor` — the core orchestration substrate).
...
It's interruptible (Esc tears it down) and bounded (default 10s, max 300s); the timeout is just the
**idle ceiling**, since it returns the instant a watched agent finishes or a message lands, so it can
never deadlock.
...
runtime finalizer does the same on exit. `:stop` interrupts one. A run's exit finalizer always fires:
an interrupted or dead sub-agent immediately **records an error return** (`[interrupted — run did not
finish]`) and `complete`s — **notifying its parent right away** through the same mailbox + completion
path a normal finish takes, so a parent waiting on it is woken even on interruption and never reads a
stranded `running`. The wait can never hang.
...
If cross-process or cross-vendor interop is ever wanted, the clean seam is the `AgentBus`
(`Supervisor`) interface plus the `run_agent` / `wait_for_agents` tools: an adapter could front an A2A
client/server there without the loop or the prompts changing. Today it's all local fibers.
