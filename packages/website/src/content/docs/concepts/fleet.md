...
on a schedule. It is **opt-in orchestration the direct agent can spin up** — not the default unit of
work. The root agent does most jobs itself in one tight loop; it reaches for a fleet only when the work
is wide or parallel enough to need one (see [direct by default](/docs/concepts/personal-assistant/#direct-by-default--delegation-is-opt-in)).
This page is the mental model and the wiring — the *how it works*. For the hands-on walkthrough, see
[Run a fleet](/docs/guides/fleet/).
...
- Parallelism is `Effect.forEach` with bounded concurrency; write-safety is the folder sandbox
  (writes confined per scope) plus **orchestrator-sequenced writes** (the lead spawns one writer at a
  time); the shared spend pool is a `Ref<number>`; an agent's ambient identity is a `FiberRef`
  (`RunContext`).
...
inside its folder, and the orchestrator sequences writers (it spawns one coder, waits, spawns the
next) rather than relying on a lock. Per-process isolation would only cost us the elegant in-memory
orchestration.
:::
...
## The bus: the core Supervisor

Coordination is `Ref`-backed (`sdk-core/usecases/agentBus.ts`). The bus *is* the **Supervisor** —
`export type Supervisor = AgentBus` — the orchestration substrate, now lifted into
`@xandreed/sdk-core`. It is a **threaded per-session value**, not a `Context.Tag` port: it's
per-session stateful and carries a per-runtime event sink (`onBusEvent`), so a `Layer`-resolved
singleton would fight that. Three pieces:
...
| Concern | Where |
| --- | --- |
| `run_agent`, fire (`spawnAgent`), the toolkit + role subset, comms handlers, the bus | `sdk-core/usecases/buildScopeRuntime.ts` |
| The comms bus / **Supervisor** (mailboxes + blackboard + supervision) | `sdk-core/usecases/agentBus.ts` |
| The live fleet registry (CLI-side) | `code/cli/state/fleet.ts` (`FleetSupervisor`) |
| Role / directive definitions (CLI) · tool / schedule definitions (core) | `code/usecases/{loadAgents,directive}.ts`, `sdk-core/usecases/{loadTools,schedule}.ts` |
| Ambient identity + per-role model override | `sdk-core/usecases/runContext.ts` → `adapters/llm/router.ts` |
| Shared token pool | `sdk-core/usecases/tokenBudget.ts` |
