...
The agent does its work itself by default; when it *does* reach for a sub-agent, delegation in efferent
is **one generic tool over a persistent branching context tree** — not pre-wired per-scope tools, not
forced prompt routing. (When to reach for one is an opt-in policy — see
[direct by default](/docs/concepts/personal-assistant/#direct-by-default--delegation-is-opt-in).)
...
- **Parallelism** — spawning is non-blocking, so multiple `run_agent` calls in one turn each fork-and-return
  at once and the whole fleet runs in parallel. Read-only work fans out freely; **writing is sequenced by
  the orchestrator** (spawn a coder, `wait_for_agents`, spawn the next) rather than a folder lock — the
  folder sandbox still confines each writer to its scope. Since the fleet are background fibers, Esc
  interrupts the root *and* the whole fleet (`bus.interruptAll`), and a runtime finalizer does the same on
  exit — no orphans.
