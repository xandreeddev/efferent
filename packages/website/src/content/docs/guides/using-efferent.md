...
efferent is a **fast, direct coder by default** — you talk to it and it reads, edits, runs, and tests
itself in one tight loop. It is also its own orchestrator: the same agent can spawn, watch, and approve
other agents, so a quick one-off session and a coordinated background fleet are the *same thing* seen
from different angles — there's no separate "fleet app" or "orchestrator mode." But the fleet is
**opt-in**: most sessions never need it, and working solo is what the agent does unless the work is wide
enough to warrant a team. This guide is the whole arc, end to end. For the focused mechanics of each
piece, follow the links; this page is the shape of the product.
...
When — and only when — the work is wider than one train of thought (three-plus independent areas, a
change across many packages, or a fire-and-forget/scheduled job), the lead agent spawns helpers — or you
fire them yourself. Each runs **alongside** your conversation (your composer stays free):
...
and **writers are sequenced by the orchestrator** (it spawns one coder, waits, spawns the next) rather
than relying on a lock. Disjoint work is safe by construction; overlapping work can't race. Reads stay
workspace-wide, so a sub-agent still learns types and conventions from anywhere in the tree.
