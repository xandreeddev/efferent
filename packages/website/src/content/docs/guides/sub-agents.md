...
The agent does most work itself; when it delegates, it's through one tool — `run_agent` — backed by a
[persistent context tree](/docs/concepts/sub-agents/). It ships in the coding agent's toolkit
(`buildScopeRuntime`); any agent built on a scope runtime gets it. (Delegation is
[opt-in](/docs/concepts/personal-assistant/#direct-by-default--delegation-is-opt-in), for wide or
parallel work.) This guide is about *using* it well.
...
- **Disjoint folders run in parallel**, write-safe by construction; concurrent writers are sequenced by
  the orchestrator (one writer at a time, not a lock); Esc interrupts the whole subtree (structured
  concurrency, no orphans).
