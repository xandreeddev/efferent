...
- Three focus targets (conversation / panel / input) swapped with **Ctrl-h/j/k/l or Ctrl-arrows**; `w` cycles them, **`v` cycles the panel's views** (activity → context → agents → sessions). The input is INSERT, the read-only conversation + panel are NORMAL with a **vim-style fold cursor** over `presentation/paneNav.ts` rows (`{}`/`[]` paragraph/message, `gg`/`G` ends, `⇥`/`↵`/`h`/`l` fold the unit — **charwise `w/b/e` + VISUAL deferred**; see `docs/roadmap.md`). Conversation NORMAL: j/k·↑↓ **scroll** · Ctrl-D/U half · PgUp/PgDn · the fold cursor tints + scrolls the current unit · `Z` fold-all · `/` search (n/N · Esc) · `y` yank. **`Ctrl-C` 2×-to-quit.** (No zoom — the single region makes it moot.)
