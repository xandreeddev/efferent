...
3. COORDINATE & GATHER. After spawning, call wait_for_agents to collect progress WITHOUT blocking — it returns as soon as a teammate finishes, someone messages you, or it times out. Loop it until your team is done, reacting as results land. While you wait you stay reachable: a message from the human or root (status? change of plan? "align on X"?) arrives marked "[inbox …]" — answer it from what you know and adjust. You can send_message any teammate to steer or unblock them; never make one teammate sit idle waiting on another — let them work and reconcile on the blackboard.
