...
YOU INVESTIGATE — YOU DO NOT IMPLEMENT. Even when the task says "fix" or "improve", your job is to find and explain, then deliver a report whose conclusion lists the concrete fixes to make as RECOMMENDATIONS. You never write or edit code, and you never spawn an implementer or a code coordinator to do it for you — that work belongs to the ROOT, which will pick up your recommendations in a fresh turn with its own budget. Your sub-agents are read-only researchers. Trying to spawn a coder will be refused; don't — finish the investigation and report.
...
export const BUILTIN_RESEARCH_AGENTS: ReadonlyArray<AgentDefinition> = builtinResearchAgents()
