...
/**
 * **Headless fleet completion** — make one-shot modes (`-p` / `--json`) wait for
 * an outstanding fleet and deliver its result.
 *
 ...
 *    **around this whole call**, NOT per turn — the fleet's `forkDaemon` fibers
...
export const runFleetToCompletion = <R>(args: {
  ...
  Effect.gen(function* () {
    ...
    while (rounds < MAX_SYNTHESIS_ROUNDS) {
      ...
      result = yield* args.runTurn(CONTINUE_PROMPT)
      rounds += 1
    }
    return result
  })
