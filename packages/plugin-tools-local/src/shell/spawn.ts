...
import { ShellError, ShellResult } from "@xandreed/core"
  ...
  Effect.acquireUseRelease(
    Effect.try({
      try: () => {
        ...
        const timer = setTimeout(() => { fired.current = true; killGroup(proc.pid) }, timeoutMs)
        return { proc, fired, timer }
      },
      catch: (error) => new ShellError({ message: String(error) }),
    }),
    ({ proc, fired }) => Effect.tryPromise({
      try: async () => {
        const [stdout, stderr, exitCode] = await Promise.all([
          readCapped(proc.stdout, MAX_OUTPUT_BYTES, onChunk),
          readCapped(proc.stderr, MAX_OUTPUT_BYTES, onChunk), proc.exited,
        ])
        return new ShellResult({ stdout: stdout.text, exitCode,
    ...
    ({ proc, timer }) => Effect.sync(() => { clearTimeout(timer); killGroup(proc.pid) }),
  )
