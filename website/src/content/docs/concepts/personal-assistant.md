---
title: The personal assistant — sessions, fleets, and agents you jump into
description: efferent's chat-first model — one master assistant per workspace that dispatches coding and research fleets, the session → fleet → agent data model, the Workspace port and its HTTP/SSE wire, and how a turn executes end to end.
sidebar:
  label: The personal assistant
  order: 10
---

efferent is a **personal engineering assistant** you chat with. It does small things itself, and for
real work it spins up a **fleet** — a coding team or a research team — that runs in the background and
reports back. Every agent in that fleet is a session you can **jump into** and talk to, as if it were a
single coherent agent. This page is the mental model end to end: the domain types, the data model, what
crosses the wire, and how a turn actually executes.

## Mental model

```
daemon  ── the per-workspace infrastructure (one process, HTTP/SSE)
  └─ session ── a master chat you resume (a conversation)
        └─ fleet ── a task the assistant dispatched (a coordinator + its team)
              └─ agent ── one worker in that fleet (a sub-agent run)
```

Two binaries share **one** chat-first TUI over the **same** `Workspace` interface:

- **`efferent`** — the master assistant. Daemon-backed. A split screen: the **chat** on the left, a
  **live fleet tree** on the right (every agent from this session, with running/idle status). N
  resumable sessions; it triages requests and dispatches coding/research fleets, does follow-ups, and
  schedules jobs.
- **`code`** — a focused, fast, single-fleet coder. In-memory (no daemon), same TUI and the same
  "jump into any agent" capability, scoped to one session.
...
A **session** the UI lists is a `SessionSummary` — the same shape whether it's the master chat, a fleet,
or a leaf agent:
...
```
conversations            ── a SESSION (the resumable master chat)
...
you type  ─▶ ws.send(sessionId, prompt)
            ...
            ─▶ inProcess.send  ──┬─ running?  → bus.post(mailbox)        (steer a live agent)
                                 ├─ busy?     → queue
                                 └─ a fleet root → startRootTurn(cid, prompt)
...
## Triage & dispatch — fleets

The assistant's system prompt carries a `# Triage and dispatch` section: do small things directly,
hand **coding** to the `coordinator` team and **deep research** to the `research-coordinator` team.
Dispatch is just the `run_agent` tool — which forks a background sub-agent and **returns immediately**:
...
```ts
// usecases/buildScopeRuntime.ts — makeRunAgentHandler (shape)
...
fiber on the `AgentBus` until a watched agent finishes, a message lands in its inbox, or a timeout —
mailboxes, a shared blackboard, and per-run completion `Deferred`s. The coordinator leads its specialists
(`frontend`/`backend`/`qa`/`product`), validates each piece with a read-only `architect`, and reports up;
the research-coordinator fans out web `researcher`s and synthesizes. See
[sub-agents](/docs/concepts/sub-agents/) and [the fleet](/docs/concepts/fleet/).
...
A fleet always reads code **fresh**, so it's never stale — but an engineer also accumulates *why* a thing
...
packages/sdk-core/src/
  ...
  usecases/buildScopeRuntime  run_agent / wait_for_agents over the AgentBus + context tree
packages/code/src/
  workspace/inProcess.ts      the authoritative Workspace (the daemon hosts it)
...
which is why a chat, a fleet you dispatch, and an agent you jump into are all the same thing wearing
different hats.
