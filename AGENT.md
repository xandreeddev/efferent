...
- **`tui.ts`** — now just the `TuiModeInput` seam; the driver lives in **`packages/cli/src/tui-solid/`** (OpenTUI native renderer + **SolidJS** signals, **no React**), loaded by `main.ts` via a lazy dynamic `import()` so `@opentui/core`'s native FFI lib is touched only on the TUI path. Default in a TTY. **Modal, multi-pane**, Neogit-flavoured layout: **two bordered boxes** (`┌─ conversation ─┐ ┌─ activity ─┐`) with **one empty column between them** (OpenTUI/Yoga flex, not hand-drawn); each pane has a **distinct accent** when focused (conversation cyan / side magenta / input green — `theme.ts`/`paneBorder`). A **bordered keybind box** (`view/chrome/Keybinds.tsx`) sits below — its border + title take the *focused* pane's accent and the title carries `<pane> · <MODE>` (the status bar is `model · tokens · storage · cwd`); its body is **two labelled rows** — a dim global `nav` row over a dynamic row of the focused pane's real keys. Plus a multi-line **`<textarea>`** input (Enter inserts a newline · **Shift-Enter** sends, Alt-Enter too · grows 1→8 rows), a `/` search status line, a dim footer, a `:` command palette (`:exit`, `:clear`, `:help`, `:cwd`, `:reset`, `:handoff`, `:context`, `:build`, `:browse`, `:resume`, `:settings`, `:set`, `:model`, `:effort`, `:search`, `:login`, `:logout`, `:db`), and `/` conversation search. Three focusable panes (conversation / side / input) swapped with **Ctrl-h/j/k/l or Ctrl-arrows**; the input is INSERT, the read-only panes NORMAL (a `mode` flag drives the legend title; **full vim modal editing — cursor motions + VISUAL — is deferred**, see roadmap). Conversation pane: **keyboard scroll** (j/k·↑↓ · Ctrl-D/U · PgUp/PgDn · gg/G) via a registered `ConvScroller`, `Z` fold-all, `/` search (jump-to-match + `[i/N]` + n/N + Esc), and **`y`** yanks the OpenTUI mouse selection via OSC 52 (drag-select is native — `useMouse:true`). The conversation is a Claude-style **event rail**: assistant prose and each tool lead with a `●` (colour carries run/ok/error), tool headers read `ToolName(arg)`, results hang under a `⎿` connector; foldable **turns** ("commits", `▾`/`▸` collapsing to `▸ <subject> · N steps`) with grouped tool calls. The default side view is **activity** — a dashboard (context gauge, cumulative output/turns/elapsed, per-LLM-call usage + duration, files-changed diffstat, foldable skills/instructions); navigable with its own block cursor (`j/k`·arrows·`gg/G`, `Tab`/`Enter`/`h`/`l`·←/→ fold a section **or** a turn/sub-agent node) and **collapses the previous run when a new user message starts a run**. The **context viewer** (`:context`) is a navigable tree of foldable, selectable **turns and handoffs**: `Tab`/`h`/`l`·←/→ fold, `Enter` jumps the conversation, `Space` selects, `b`/`:build` builds a new session from the selection. **Ctrl-C** is 2×-to-quit. Structure under `packages/cli/src/tui-solid/`: `runtime.ts` (composition root + the three-runtime bridge), `state/` (signal slices), `events/` (Effect→signal pump), `actions/` (signal→Effect), `keys/` + `commands/`, `model/`, `view/` (`panes/`, `panes/side/`, `chrome/`, `overlays/`). Several **pure** helpers are still reused from `tui/` (`contextView`, `sidePane`, `selectBox`, `loginFlow`, `settingsView`, `slashPalette`, `statusBar`, `executionTree`, `toolDescribe`, `dbStatus`, `terminal` (OSC/spinner consts), `logger`).
...
- **Full vim modal editing in the TUI** — deferred at cutover (the OpenTUI/Solid TUI ships scroll/`/`-search/fold-all/mouse-yank but not the vim cursor). To restore: a per-line cursor + NORMAL/VISUAL motions (`w/b/e`, `0/^/$`, `{/}` turn jumps, charwise/linewise VISUAL) layered on the read-only panes + the `<textarea>` (reuse the pure `viMode`/`navKeys` logic, which was deleted with the old driver — recover from git history). Related deferred TUI bits: **markdown rendering of assistant prose** (plain text today — the old `markdown→ANSI` was deleted at cutover; OpenTUI ships native `<markdown>`/`<code>` tree-sitter components to adopt), **per-turn fold-under-cursor** (needs that cursor), **prompt-history recall** on ↑/↓ (collides with the textarea's line navigation — intercept at the first/last line), the **startup conversation picker** (have `:browse`/`:resume`), **Ctrl-R tool-output expand**, the **Shift-Tab effort shortcut**, and **per-tool detail folding** (turn + tool-group folding is per-section today).
