...
| `run_agent`  | `{ folder; task; seedFromNode?; seedMode? }`     | spawns a folder-scoped sub-agent (see Sub-agents & context tree) |
...
- **Resume / branch**: `seedFromNode` + `seedMode: "resume"` appends the task to the existing node and continues in its context (same node grows); `"branch"` spawns a **child** node (`edgeKind: "branched"`) seeded from the source node's messages. So the parent can keep using a sub-agent's context or fork it — the tree records how context branched and evolved.
- **Identity & bounds**: `RunContextRef` (a `FiberRef` in `usecases/runContext.ts`) carries `{ rootConversationId, parentNodeId, depth }` ambiently — `runAgent` seeds it via `Effect.locally`, each spawn re-seeds for its children. A `maxDepth` guard (default 6) returns `MaxDepthReached` as a tool failure; each sub-agent run is bounded by its own `maxSteps`.
...
- **TUI** — **`:tree`** toggles the **agent context tree** in the side pane (third view next to activity + `:context`): a navigable, foldable tree of every persisted node — folder, status glyph (`✓`/`✗`/`●`), provenance (`spawned`/`branched`/`resumed`), seed kind, files count, return summary. Same paneNav motions; `d` drops a node + descendants (running nodes guarded); refreshes live as a run spawns sub-agents (`presentation/contextTreeView.ts`, `view/panes/side/ContextTree.tsx`, `actions/contextTree.ts`).
...
- **Human-driven resume/branch from `:tree`** — the agent resumes/branches sub-agent contexts via `run_agent` today; a key in the tree view that *re-runs* a node (pick seed → submit-level run wiring) is the missing half. `d` (drop) ships.
