...
- **`tui.ts`** — default in a TTY. Hand-rolled **modal, multi-pane**, Neogit-flavoured layout: **two separate bordered boxes** (`┌─ conversation ─┐ ┌─ activity ─┐`) with **one empty column between them**; each pane has a **distinct accent** when focused (conversation cyan / side magenta / input green — `PANE_ACCENT` in `render.ts`). A **bordered keybind box** (`legend.ts`) sits below — its border + title take the *focused* pane's accent and the title carries `<pane> · <MODE>` (the only place the vim mode shows; the status bar is `model · tokens · storage · cwd`, where `storage` is the active store `sqlite`/`pg`); its body is **two labelled rows** — a dim `nav` row (global movement: pane switching / `:` / `/` / zoom, identical in every pane) over a dynamic row of the focused pane's own keys. Plus a multi-line input box, a dim footer (logs path + key hints), a `:` command palette (`:exit`, `:clear`, `:help`, `:cwd`, `:reset`, `:handoff`, `:context`, `:build`, `:browse`, `:resume`, `:settings`, `:set`, `:model`, `:login`, `:logout`, `:db`) and a `/` search. Three focusable panes (conversation / side / input) swapped with **Ctrl-h/j/k/l or Ctrl-arrows**; **INSERT** only on the input, **NORMAL + VISUAL** on the read-only panes. The cursor is a **real nvim block cursor** (the hardware terminal cursor — block in NORMAL/VISUAL, bar in INSERT) with full motions (`h/j/k/l` + arrows, `w/b/e`·`W/B/E`, `0/^/$`, Home/End, `gg/G`, `{/}` turns, `Ctrl-D/U`); `v`/`V` charwise/linewise VISUAL + `y` yank (OSC 52). The conversation is a Claude-style **event rail**: assistant prose and each tool lead with a `●` (its colour carries run/ok/error), tool headers read `ToolName(arg)`, and the result hangs under a `⎿` connector with the diff/output beneath; it's still a tree of **foldable turns** ("commits", `▾`/`▸` chevron + the prompt, collapsing to `▸ <subject> · N steps`) with grouped tool calls — `Tab`/`Enter` fold the section under the cursor, `Z` all. The default side view is **activity** — an at-a-glance dashboard (context gauge, cumulative output/turns/elapsed, per-LLM-call token usage + duration per turn, a files-changed diffstat, and foldable skills/instructions sections that show a key-info summary when collapsed); navigable with its own block cursor (`j/k`·arrows·`gg/G`, `Tab`/`Enter`/`h`/`l`·←/→ fold the row — a section **or** a turn/sub-agent node) and **collapses the previous run's turns/sub-agents when a new user message starts a run** (each run begins clean; manual folds within a run persist). A sub-agent's `subagent_end` carries its cumulative token usage, shown in the node detail (`<ctx>` / `<out>`). The **context viewer** (`:context`) is a navigable tree of foldable, selectable **turns and handoffs** with its own block cursor: `Tab`/`h`/`l`·←/→ fold a turn or handoff segment, `Enter` jumps the conversation to it, `Space` selects a turn or a handoff (its summary), `b`/`:build` builds a new session from the selection. No React, no Ink, no blessed — just `Bun + ANSI` in `packages/cli/src/tui/`.
...
- **Image attachments** in the CLI; **mouse support** in the TUI (intentionally none — the TUI stays out of mouse-reporting mode so terminal-native click-drag selection keeps working; navigation/selection is keyboard-modal: Ctrl-hjkl/Ctrl-arrows panes, j/k·arrows·gg/G·{/} scroll, `/` search, `v`+`y` yank); **native (non-shell-out) grep**.
