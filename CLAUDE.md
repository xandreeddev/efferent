...
- **`tui.ts`** — default in a TTY. Hand-rolled **modal, multi-pane**, Neogit-flavoured layout: **bordered boxes** (`┌─ conversation ─┬─ context ─┐`, the focused box's border brightens), an **always-on per-pane legend** (`legend.ts` — a fixed keybind cheatsheet), a multi-line input, a `:` command palette (`:exit`, `:clear`, `:help`, `:cwd`, `:reset`, `:handoff`, `:context`, `:browse`, `:resume`, `:settings`, `:set`, `:model`) and a `/` search. Three focusable panes (conversation / side / input) swapped with **Ctrl-h/j/k/l**; **INSERT** only on the input, **NORMAL + VISUAL** on the read-only panes. The cursor is a **real nvim block cursor** (the hardware terminal cursor — block in NORMAL/VISUAL, bar in INSERT) with full motions (`h/j/k/l`, `w/b/e`·`W/B/E`, `0/^/$`, `gg/G`, `{/}` turns, `Ctrl-D/U`); `v`/`V` charwise/linewise VISUAL + `y` yank (OSC 52). The conversation is a tree of **foldable turns** ("commits") with grouped tool calls — `Tab`/`Enter` fold the section under the cursor, `Z` all. The **context viewer** (`:context`) is a navigable tree with its own block cursor: `Tab`/`h`/`l` fold a handoff segment, `Enter` jumps the conversation to a message. No React, no Ink, no blessed — just `Bun + ANSI` in `packages/cli/src/tui/`.
